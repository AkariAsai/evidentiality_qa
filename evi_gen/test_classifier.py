    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            for k, o in enumerate(outputs):
                ...
                print(ans)
                print( [dataset[idx[k]]['target']])
                if 'answers' in example:
                    score = src.evaluation.ems(ans, [dataset[idx[k]]['target']])
...
if __name__ == "__main__":
    ...
    collator_function = src.data.CollatorClassification(opt.text_maxlength, tokenizer)
    eval_examples = src.data.load_data_class(
        opt.eval_data, 
        global_rank=opt.global_rank, #use the global rank and world size attibutes to split the eval set on multiple gpus
        world_size=opt.world_size
    )
    eval_dataset = src.data.DatasetClassification(
        eval_examples, 
        opt.n_context, 
    )
    ...
    if opt.joint is True:
        # copy joint model's decoder and class_lm weights.
        print("over loading weights")
        model_class = src.model.FiDT5ContrastiveJoint
        model = model_class.from_pretrained(opt.model_path)
        model.copy_class_decoder_weights(model.state_dict())
