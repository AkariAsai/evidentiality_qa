...
import numpy as np
from datasets import load_dataset, load_metric, GenerateMode
...
def main():
    ...
    if training_args.do_predict:
        ...
        for test_dataset, task in zip(test_datasets, tasks):
            ...
            scores = copy.deepcopy(predictions)
            predictions = np.squeeze(
                predictions) if is_regression else np.argmax(predictions, axis=1)

            # output label and scores
            final_result = {}
            for index, (pred, score) in enumerate(zip(predictions, scores)):
                print(print)
                print(score)
                final_result[index] = {"pred": label_list[pred], "score": [
                    float(score[0]), float(score[1])]}
            with open(os.path.join(training_args.output_dir, "test_pred_scores.json"), 'w') as outfile:
